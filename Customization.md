...
You can also override the `rem` function by defining a `rem-custom` function which would be used instead for calculating the size of components components. Please refer to the original function in [`keen-ui/src/styles/utils.scss`](.../src/styles/utils.scss).
